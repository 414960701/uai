...
WHEN 同一 Agent frame 内完全相同的工具名称和参数重复调用
THE SYSTEM SHALL 在重复达到防线时发出公开、有界的 `repeated_tool_call` 提示，并向模型
注入要求复用结果、提出新问题或进入实施/验证的运行时提示；该提示不得设置固定的全局
步数/研究次数上限，也不得阻止参数或目标确实不同的正常多阶段工作。
