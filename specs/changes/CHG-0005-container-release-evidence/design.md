...
2. Web 镜像构建阶段裁剪依赖并执行 production audit；启动后轮询两个容器的 Docker
   ...
   开发工具，再运行后端 doctor。
3. 运行容器内 doctor，并通过控制 API 验证新数据库为空、provider 注册表只包含
   `openai_compatible`，不写入示例 Agent、凭据、模型档或运行记录。
4. 清理并断言容器、network、volume 均不存在，再输出不含 Secret 的摘要。

Makefile 的 `container-smoke` 目标直接调用该脚本。Control Center 只把“单节点容器”标为已验证，
...
钉住已修复的传递依赖。`package-lock.json` 是本地发布门禁与容器 `npm ci` 的唯一解析结果。
