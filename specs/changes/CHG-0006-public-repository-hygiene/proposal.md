...
- 从本地发布门禁、Docker build context 和 Dockerfile 中移除对个人 hosting 文件的依赖。
- 扫描当前树和 Git 历史中的账号标识、本机路径及常见凭据签名。

## 非目标

- 删除仓库内自动化 workflow；公开仓库只保留可由维护者显式运行的发布门禁脚本。
- 不重写 Git 历史；已公开的 Sites `project_id` 不是 Secret，当前树移除即可。
- 不改变 Agent Runtime、API、插件协议、数据模型或部署成熟度声明。

## 实现证据

2026-07-31 的干净 checkout 形态前端门禁、后端测试、production audit、配置检查和
